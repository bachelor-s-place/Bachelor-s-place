-- =============================================================================
-- BachelorsSpace — Schema Initializer
...
CREATE TYPE property_gender_pref AS ENUM (
    'male',    -- male tenants only
    'female',  -- female tenants only
...
CREATE TYPE squad_status AS ENUM (
    ...
    'pending_landlord_approval', -- Waiting for landlord to accept
    'payment_pending', -- Landlord approved, waiting for token payment
...
-- personality_embedding vector dimensions depend on the EMBEDDING_PROVIDER:
--   ollama (mxbai-embed-large): vector(1024)   ← current dev default
--   openai (text-embedding-3-small): vector(1536)
-- The value below MUST match EMBEDDING_MODEL in .env.
-- -----------------------------------------------------------------------------
CREATE TABLE users (
    ...
    whatsapp_encrypted      TEXT,                               -- AES-256 encrypted; optional landlord WhatsApp
    role                    user_role       NOT NULL,
    gender                  gender_type,                        -- optional; NULL = not disclosed; tenant-relevant only
    personality_embedding   vector(1024),                       -- 1024 = mxbai-embed-large (Ollama); change to 1536 for OpenAI
    ...
    embedding_attempts      INT             NOT NULL DEFAULT 0,    -- retry counter for the embedding worker
    embedding_next_attempt_at TIMESTAMPTZ,                         -- backoff: don't retry before this time
...
CREATE TABLE properties (
    ...
    total_capacity   INT,
    lifestyle_tags   TEXT[]                DEFAULT '{}',
    for_gender       property_gender_pref  NOT NULL DEFAULT 'any', -- who this property is listed for
    token_percentage NUMERIC(4,2)          NOT NULL DEFAULT 5.00,  -- % of rent as token; 0–15
    ...
    -- Token percentage must be between 0 and 15
    CONSTRAINT token_pct_range CHECK (
        token_percentage >= 0 AND token_percentage <= 15
    )
);
...
CREATE TABLE squads (
    ...
    gender                  gender_type,                                 -- single-gender invariant; set from the creator by trigger
...
-- Single-gender invariant backstop: every accepted member must share the squad's
-- gender. The first accepted member (the squad creator) defines it; later accepted
-- members are rejected on mismatch. This is a DB-level fail-safe behind the
-- application-layer checks in the squad/transaction services.
...
BEGIN
    ...
    IF v_squad_gender IS NULL THEN
        -- First accepted member (creator) sets the squad's gender.
...
-- -----------------------------------------------------------------------------
-- property_inquiries / inquiry_messages
-- Contact-masked tenant<->landlord pre-booking chat about a specific property.
-- No contact details are exposed here; phone reveal stays gated behind BR-06.
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_property_inquiries_tenant    ON property_inquiries (tenant_id, last_message_at DESC);
CREATE INDEX idx_property_inquiries_landlord  ON property_inquiries (landlord_id, last_message_at DESC);
