...
import (
	...
	"bachelorsSpace/internal/handler"
...
func main() {
	...
	case "mock":
		embedder = embedding.NewMockProvider()
		log.Info().Msg("starting embedding worker with MOCK provider")
	default:
		// Fallback to mock provider if OLLAMA_HOST is not configured to avoid unsupported protocol errors
		if os.Getenv("OLLAMA_HOST") == "" {
			embedder = embedding.NewMockProvider()
			log.Info().Msg("OLLAMA_HOST not configured — automatically initialized MOCK embedding provider for development")
		} else {
			embedder = embedding.NewOllamaProvider(os.Getenv("OLLAMA_HOST"), os.Getenv("EMBEDDING_MODEL"))
		}
	}
