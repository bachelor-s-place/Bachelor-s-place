...
func (m *Mailer) send(to, subject, body string) error {
	...
	// Strip CR/LF from header values to prevent SMTP header injection
	// (e.g. a crafted "to"/"subject" smuggling extra Bcc/headers).
	to = sanitizeHeader(to)
	subject = sanitizeHeader(subject)

	msg := strings.Join([]string{
		fmt.Sprintf("From: BachelorsSpace <%s>", sanitizeHeader(m.fromAddr)),
		fmt.Sprintf("To: %s", to),
...
// sanitizeHeader removes CR and LF characters so a value cannot break out of its
// header line and inject additional SMTP headers.
