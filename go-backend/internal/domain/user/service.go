...
type Repository interface {
	...
	UpdateContact(ctx context.Context, userID string, phoneEnc string, whatsappEnc *string) error
}

// Encryptor is the subset of crypto.Encryptor the user service needs for PII fields.
...
func NewService(repo Repository, jwtSecret, googleClientID string, encryptor Encryptor) *Service {
	return &Service{
		repo:           repo,
...
// UpdateContact encrypts and stores the user's contact phone and optional WhatsApp
// number. These are revealed to a squad only after it locks a property (BR-06) and
// are used by the inquiry channel; they are never returned in plain profile responses.
func (s *Service) UpdateContact(ctx context.Context, userID string, input UpdateContactInput) error {
	phone := strings.TrimSpace(input.Phone)
	if phone == "" {
		return ErrContactRequired
	}
	phoneEnc, err := s.encryptor.Encrypt(phone)
	if err != nil {
		return fmt.Errorf("user service: failed to encrypt phone: %w", err)
	}

	var whatsappEnc *string
	if input.Whatsapp != nil {
		if w := strings.TrimSpace(*input.Whatsapp); w != "" {
			enc, err := s.encryptor.Encrypt(w)
			if err != nil {
				return fmt.Errorf("user service: failed to encrypt whatsapp: %w", err)
			}
			whatsappEnc = &enc
		}
	}

	return s.repo.UpdateContact(ctx, userID, phoneEnc, whatsappEnc)
}
