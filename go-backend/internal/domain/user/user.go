...
type User struct {
	...
	WhatsappEncrypted   *string  `json:"-"` // AES-256 — never expose raw
	...
	// Derived booleans so the client knows whether contact details are on file
	// without ever exposing the encrypted values themselves.
...
// UpdateContactInput is the validated request body for PUT /api/v1/users/me/contact.
// Landlords must provide a phone number; WhatsApp is optional.
type UpdateContactInput struct {
	Phone    string  `json:"phone"    validate:"required,min=8,max=20"`
	Whatsapp *string `json:"whatsapp" validate:"omitempty,min=8,max=20"`
}
...
var (
	...
	ErrContactRequired    = errors.New("user: a contact phone number is required")
)
...
func ToAPIError(err error) *apierror.APIError {
	...
	case errors.Is(err, ErrContactRequired):
		return apierror.ValidationError("a contact phone number is required")
