...
func (s *Service) GetSquadDetails(ctx context.Context, userID, squadID string) (map[string]interface{}, error) {
	...
	// BR-06: Reveal landlord contact only once the squad is locked or moved_in.
	if sq.Status == StatusLocked || sq.Status == StatusMovedIn {
		if sq.PropertyID != nil {
			if contact := s.landlordContact(ctx, *sq.PropertyID); contact != nil {
...
// landlordContact loads and decrypts the property owner's contact details.
// Returns nil if it can't be loaded (e.g. no contact on file), so callers simply omit it.
func (s *Service) landlordContact(ctx context.Context, propertyID string) map[string]string {
	name, phoneEnc, whatsappEnc, err := s.repo.GetLandlordContact(ctx, propertyID)
	if err != nil {
		return nil
	}

	contact := map[string]string{"name": name}
	if phoneEnc != nil && s.encryptor != nil {
		if phone, err := s.encryptor.Decrypt(*phoneEnc); err == nil {
			contact["phone"] = phone
		}
	}
	if whatsappEnc != nil && s.encryptor != nil {
		if wa, err := s.encryptor.Decrypt(*whatsappEnc); err == nil {
			contact["whatsapp"] = wa
		}
	}
	return contact
