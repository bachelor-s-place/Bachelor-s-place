...
// Encryptor is the subset of crypto.Encryptor the squad service needs to decrypt the
// landlord's contact details before revealing them to a locked squad.
...
type Repository interface {
	...
	// GetLandlordContact returns the property owner's name and their ENCRYPTED phone and
	// WhatsApp (nil when not set). The service decrypts before exposing them (BR-06).
	GetLandlordContact(ctx context.Context, propertyID string) (name string, phoneEnc, whatsappEnc *string, err error)
