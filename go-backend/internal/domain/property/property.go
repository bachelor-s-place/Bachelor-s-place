...
var (
	...
	ErrContactRequired  = errors.New("property: a contact number is required to list")
)
...
func ToAPIError(err error) *apierror.APIError {
	...
	case errors.Is(err, ErrContactRequired):
		return apierror.New(http.StatusUnprocessableEntity, "CONTACT_REQUIRED", "add a contact number in your profile before listing a property")
	default:
