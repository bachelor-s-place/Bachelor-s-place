...
func NewRouter(pool *pgxpool.Pool, jwtSecret string, encryptor *crypto.Encryptor, cfg *config.Config) http.Handler {
	...
	userService := user.NewService(userRepo, jwtSecret, cfg.GoogleClientID, encryptor)
	...
	squadService := squad.NewService(squadRepo, encryptor)
	...
	// Tenant<->landlord inquiry channel (contact-masked)
	inquiryRepo := repository.NewInquiryRepo(pool)
	inquiryService := inquiry.NewService(inquiryRepo)
	inquiryHandler := NewInquiryHandler(inquiryService)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Tenant<->landlord inquiries (contact-masked pre-booking chat)
			r.Mount("/inquiries", inquiryHandler.Routes())
		})
