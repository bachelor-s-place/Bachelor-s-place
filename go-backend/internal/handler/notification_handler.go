...
func (h *NotificationHandler) MarkOneRead(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.repo.MarkOneRead(r.Context(), notifID, userID); err != nil {
		// Only a genuine "no such notification for this user" is a 404; a real DB
		// failure must surface as 500, not be masked as not-found.
		if errors.Is(err, pgx.ErrNoRows) {
			respond.Error(w, apierror.NotFound("notification not found"))
			return
		}
		respond.Error(w, apierror.Internal("failed to mark notification as read"))
		return
	}
