...
// UpdateContact stores the encrypted contact phone (required) and WhatsApp (optional).
func (r *UserRepo) UpdateContact(ctx context.Context, userID string, phoneEnc string, whatsappEnc *string) error {
	const query = `
		UPDATE users
		SET    phone_encrypted = $1,
		       whatsapp_encrypted = $2
		WHERE  id = $3
		  AND  deleted_at IS NULL`

	cmd, err := r.pool.Exec(ctx, query, phoneEnc, whatsappEnc, userID)
	if err != nil {
		return err
	}
...
// Gender is updated only when explicitly provided (non-nil).
