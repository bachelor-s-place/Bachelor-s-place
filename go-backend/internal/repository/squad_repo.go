...
func (r *SquadRepo) GetLandlordContact(ctx context.Context, propertyID string) (string, *string, *string, error) {
	const query = `
		SELECT u.name, u.phone_encrypted, u.whatsapp_encrypted
		FROM   properties p
		JOIN   users u ON p.owner_id = u.id
		WHERE  p.id = $1`

	var name string
	var phoneEnc, whatsappEnc *string
	err := r.pool.QueryRow(ctx, query, propertyID).Scan(&name, &phoneEnc, &whatsappEnc)
	if err != nil {
		return "", nil, nil, err
	}
	return name, phoneEnc, whatsappEnc, nil
}
