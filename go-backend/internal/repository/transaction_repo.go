...
// MarkSuccessIfPending flips status to 'success' only when it is not already
// 'success'. The WHERE guard makes this the atomic idempotency boundary for
// concurrent/replayed webhook deliveries: exactly one caller gets RowsAffected == 1.
func (r *TransactionRepo) MarkSuccessIfPending(ctx context.Context, id string, rawStatus string) (bool, error) {
	const query = `
		UPDATE transactions
		SET status = 'success', gateway_status = $1, settled_at = NOW()
		WHERE id = $2::UUID AND status <> 'success'`

	tag, err := r.pool.Exec(ctx, query, rawStatus, id)
	if err != nil {
		return false, err
	}
