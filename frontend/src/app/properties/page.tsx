...
import { Property, PropertySearchFilter } from '@/types/property';
...
export default function PropertiesPage() {
  ...
  const fetchProperties = useCallback(async (currentFilters: PropertySearchFilter) => {
    ...
    try {
      ...
      setError(errMsg || 'Failed to fetch properties');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    Promise.resolve().then(() => fetchProperties(filters));
  ...
  useEffect(() => {
    if (!authLoading && user?.gender && !filters.for_gender) {
      Promise.resolve().then(() => setFilters(prev => ({ ...prev, for_gender: user.gender })));
    }
  }, [user, authLoading, filters.for_gender]);
