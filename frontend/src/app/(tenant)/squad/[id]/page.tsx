...
interface SquadDetailsResponse {
  ...
  // Present only once the squad is locked/moved_in (BR-06).
  landlord_contact?: { name?: string; phone?: string; whatsapp?: string };
}
...
function SquadDetailsContent() {
  ...
  const fetchSquadDetails = useCallback(async () => {
    try {
      ...
      setError(errMsg || 'Failed to load squad details');
    } finally {
      setLoading(false);
    }
  }, [squadId]);
  ...
  useEffect(() => {
    if (squadId) {
      Promise.resolve().then(() => fetchSquadDetails());
    }
  }, [squadId, fetchSquadDetails]);
  ...
  const { squad, members, proposals, landlord_contact } = data;
  ...
  return (
    <main className={styles.container}>
      <div className={styles.content}>
        ...
        {/* Landlord contact — revealed only after the squad locks the property (BR-06) */}
        {landlord_contact && (landlord_contact.phone || landlord_contact.whatsapp) && (
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <MessageSquare size={16} style={{ color: '#6c63ff' }} /> Landlord Contact
            </div>
            {landlord_contact.name && (
              <div style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)', marginBottom: '0.5rem' }}>{landlord_contact.name}</div>
            )}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem' }}>
              {landlord_contact.phone && (
                <a href={`tel:${landlord_contact.phone}`} className={styles.primaryBtn} style={{ textDecoration: 'none' }}>
                  📞 Call {landlord_contact.phone}
                </a>
              )}
              {landlord_contact.whatsapp && (
                <a
                  href={`https://wa.me/${landlord_contact.whatsapp.replace(/[^0-9]/g, '')}`}
                  target="_blank"
