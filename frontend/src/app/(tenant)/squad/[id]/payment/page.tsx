...
export default function PaymentPage() {
  ...
  useEffect(() => {
    if (window.Razorpay) { setTimeout(() => setRazorpayReady(true), 0); return; }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(errMsg || 'Failed to load details');
      } finally {
        setLoading(false);
      }
    };
  ...
  let baseRent = (property?.rent_amount as number) || 0;
  if (property?.property_type === 'pg' && selectedRoomId) {
    const room = rooms.find(r => r.id === selectedRoomId);
    if (room) baseRent = room.rent_amount as number;
  }

  const tokenPercentage = (property?.token_percentage as number) || 5;
  const tokenAmount = (baseRent * tokenPercentage) / 100;
  const memberCount = (squad?.current_member_count as number) || 1;
  ...
  const handlePayment = async () => {
    ...
    try {
      const payload: Record<string, unknown> = { payment_model: paymentModel };
      ...
      rzp.on('payment.failed', function (response: unknown) {
        setError((response as { error?: { description?: string } })?.error?.description || 'Payment failed. Please try again.');
        submittingRef.current = false;
        setProcessing(false);
      });
      rzp.open();
      ...
      const errMsg = err instanceof Error ? err.message : (err as { message?: string })?.message;
      setError(errMsg || 'Failed to initiate payment');
      submittingRef.current = false;
      setProcessing(false);
    }
  };
    ...
    return (
      <main className={styles.container}>
        <div className={styles.loginCard} style={{ textAlign: 'center', maxWidth: '400px' }}>
          ...
          <p className={styles.subtitle}>This squad is not in a state to make a token payment. Current status: {String(squad.status).replace('_', ' ')}</p>
          <Link href={`/squad/${id}`} className={styles.submitBtn} style={{ display: 'inline-block', textDecoration: 'none', marginTop: '1rem' }}>
            ← Back to Squad
          </Link>
        </div>
      ...
      <main className={styles.container}>
        <div className={styles.loginCard} style={{ textAlign: 'center', maxWidth: '460px' }}>
          ...
          <p className={styles.subtitle} style={{ marginBottom: '2rem' }}>
            Your token payment was successful. <strong>{String(property.title)}</strong> is now reserved for your squad.
    ...
    <main className={styles.container}>
      ...
      <div className={styles.loginCard} style={{ maxWidth: '500px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '12px', marginBottom: '1.5rem' }}>
          <h3 style={{ color: '#fff', fontSize: '1.1rem', marginBottom: '0.5rem' }}>{String(property.title)}</h3>
          ...
          {property.property_type === 'pg' && rooms.length > 0 && (
            <div className={styles.formGroup} style={{ marginTop: '1rem' }}>
              ...
              >
                {rooms.map(r => (
                  <option key={r.id as string} value={r.id as string}>{String(r.room_type)} (₹{Number(r.rent_amount)}/mo)</option>
                ))}
              </select>
            </div>
          )}
