...
export default function Dashboard() {
  ...
  const fetchMatches = useCallback(async (pageNum: number, isInitial = false) => {
    ...
    try {
      ...
      if (errMsg.toLowerCase().includes('not found')) {
        ...
        setError(errMsg || 'Failed to fetch matches.');
      }
    } finally {
      setLoading(false);
  ...
  const handleInviteResponse = async (squadId: string, action: 'accept' | 'reject') => {
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : (err as { message?: string })?.message;
      alert(errMsg || `Failed to ${action} invite`);
    } finally {
    ...
    <main className={styles.container}>
      ...
      <section className={styles.mainContent}>
        ...
        {!loading && !error && !mySquadId && matches.length > 0 && (
          <div className={styles.profileBanner} style={{ marginBottom: '1.5rem', background: 'var(--warning-tint)', border: '1px solid var(--warning-border)', backdropFilter: 'var(--blur-2)', WebkitBackdropFilter: 'var(--blur-2)' }}>
            <div>
              <div className={styles.bannerTitle} style={{ color: 'var(--warning)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <Link href="/squad">
              <button className={styles.secondaryBtn} style={{ flexShrink: 0, whiteSpace: 'nowrap', borderColor: 'var(--warning-border)', color: 'var(--warning)' }}>
                Create Squad →
              </button>
            </Link>
          </div>
